            ...
            while (true) {
                ...
                if (o != null) {
                    if (o instanceof LoginDTO) {
                        ...
                        String player = (String) pr.getValue();

                        System.out.println("NAME: "+player);
                        if(c.searchPlayerByName(player)==null){
                            Player p = server.getDatabase().searchPlayerByName(player);
                            p.setClub(c.getName());
                            c.addPlayer(p);
                            server.getDatabase().getOnSell().remove(p);
                        }
                        else{
                            Player p = c.searchPlayerByName(player);
